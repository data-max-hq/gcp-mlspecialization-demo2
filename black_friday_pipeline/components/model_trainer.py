...
def input_fn(file_pattern, tf_transform_output, batch_size=200):
        ...
        def add_sample_weights(features, label):
            ...
            conditions = [
                (tf.equal(age_index, AGE_GROUP_INDICES['0-17']), lambda: tf.constant(AGE_GROUP_WEIGHTS[0])),
                (tf.equal(age_index, AGE_GROUP_INDICES['55+']), lambda: tf.constant(AGE_GROUP_WEIGHTS[6]))
            ]
